...
- Maintain both Codex and Claude Code packaging for every bundle, with the same skills available in each. Document installation for both hosts prominently.
- `.agents/plugins/marketplace.json` and `plugins/*/.codex-plugin/plugin.json` are the primary packaging sources. Generate Claude compatibility metadata with `python3 scripts/sync_claude_compat.py`; do not duplicate skill bodies or recreate a commands-only implementation.
